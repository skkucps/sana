...
void TMAC::initialize(int stage) {
	...
	if (stage == 0) {
		...
		/***********************
		 * 2014_03_03
		 * Sangsoo Jeong : Custom initializing value is here
		 */
		nextMacEvent = new cMessage("next Mac Event");
		...
		if(rsu == 0)
		{
		    m_iAccessMode = AM_RSU;
		    rsu++;
		    scheduleAt(simTime()+0.1,nextTAFEvent);
		}
		else
		    m_iAccessMode = AM_VEHICLE_CP;

		m_iMACAddrArraySize = 20;

		EV << "SIZE2 : " << sizeof(WaveShortMessage) << " : " << sizeof(TAF) << endl;
	}
}

void TMAC::handleSelfMsg(cMessage* msg) {
...
//	        SendInCFP();
	...
	else if(msg == nextTAFEvent)
	{
	    /**********
	     * 2014_03_04
	     * Sangsoo Jeong : TAF message will be generated on every 0.1s
	     */
	    TAF* taf = prepareTAF();
	    cMsg*   msg =   new cMsg(MT_TAF,(cMessage*)taf);
//	    m_oMessageQueue.push(msg);
	    SendInCFP(msg);

	    scheduleAt(simTime()+SimTime().setRaw(1000000000000UL) ,nextTAFEvent);  //0.1s
	}
}

void TMAC::handleUpperControl(cMessage* msg) {
	assert(false);
}

void TMAC::handleUpperMsg(cMessage* msg) {
	...
	//rewrite SCH channel to actual SCH the TMAC is set to
	...
	/********************************************
	 * 2014_03_03
	 * Sangsoo Jeong : I add queue for CFP which stores both WSM and TAF in AM_RSU
	 *
	 */
	if(m_iAccessMode    !=  AM_RSU || m_iAccessMode ==  AM_VEHICLE_CFP)
	{
	    cMsg *pMsg=   new cMsg(MT_WSM,(cMessage*)thisMsg);
	    m_oMessageQueue.push(pMsg);
	}
	else
	{
	    EV  <<  "Data2\n";
...
void TMAC::handleLowerControl(cMessage* msg) {
...
void TMAC::setActiveChannel(t_channel state) {
	activeChannel = state;
...
void TMAC::attachSignal(Mac80211Pkt* mac, simtime_t startTime, double frequency) {
...
Signal* TMAC::createSignal(simtime_t start, simtime_t length, double power, double bitrate, double frequency) {
...
simtime_t TMAC::timeLeftTillGuardOver() const {
	ASSERT(useSCH);
...
simtime_t TMAC::timeLeftInSlot() const {
	ASSERT(useSCH);
...
void TMAC::changeServiceChannel(int cN) {
	ASSERT(useSCH);
...
void TMAC::handleLowerMsg(cMessage* msg) {
	...
	if (macPkt->getDestAddr() == myMacAddress) {
		...
		/********************8
		 * 2014_03_05
		 * Sangsoo Jeong : below if condition is added because now there are two types of data wsm and taf.
		 * It is different from previous system using only wsm.
		 *
		 */

		if(std::string(macPkt->getName()) == "Data")
		    sendUp(wsm);
		else if(std::string(macPkt->getName()) == "TAF")
		{
		    EV << "TAF : "  <<  taf <<  endl;
		    OnTAF(taf);
		}
...
int TMAC::EDCA::queuePacket(t_access_category ac,WaveShortMessage* msg) {
...
int TMAC::EDCA::createQueue(int aifsn, int cwMin, int cwMax,t_access_category ac) {
...
TMAC::t_access_category TMAC::mapPriority(int prio) {
...
WaveShortMessage* TMAC::EDCA::initiateTransmit(simtime_t lastIdle) {
...
simtime_t TMAC::EDCA::startContent(simtime_t idleSince,bool guardActive) {
...
void TMAC::EDCA::stopContent(bool allowBackoff, bool generateTxOp) {
	//update all Queues
...
void TMAC::EDCA::backoff(t_access_category ac) {
...
void TMAC::EDCA::postTransmit(t_access_category ac) {
...
void TMAC::channelBusySelf(bool generateTxOp) {
...
void TMAC::channelIdle(bool afterSwitch) {
...
void TMAC::checkBitrate(int bitrate) const {
 ...
 * 2014_03_03
 * Sangsoo Jeong:
 * Additional function will be located below
 */

TAF*    TMAC::prepareTAF()
{
//    WaveShortMessage* taf = new WaveShortMessage("TAF")
    TAF*    taf = new TAF("TAF");
    t_channel channel = false ? type_SCH : type_CCH;

    taf->addBitLength(256);
    taf->addBitLength(3000);
    EV  <<  "TAFLen : "    <<  taf->getBitLength()<<  endl;
    switch (channel) {
        case type_SCH: taf->setChannelNumber(Channels::SCH1); break; //will be rewritten at Mac1609_4 to actual Service Channel. This is just so no controlInfo is needed
        case type_CCH: taf->setChannelNumber(Channels::CCH); break;
    }

    taf->setTimestamp(simTime());
    taf->setCapability(0);
    taf->setSupportedRates(0);
    taf->setCountry(0);
    taf->setEDCAParameterSet(0);
    taf->setPowerConstraint(0);
    taf->setTiming(0);
    taf->setExtendedCapabilities(0);

    SimTime CFPMax = SIFS_11P;
    for(int i = 0 ; i < 20 ; i++)
    {
        if(m_iParticipants[i] == 0)
            break;
        CFPMax += SLOTLENGTH_11P;
    }
    taf->setCFPMAXDuration(CFPMax);
    taf->setSuperframeDuration(0.01);
    EV  << "CFPMax : " << CFPMax << " " << taf->getCFPMAXDuration() << endl;
    EV << "m_iMAC : " << m_iMACAddrArraySize << endl;
    taf->setMACAddressArraySize(m_iMACAddrArraySize);
    for(int i = 0 ; i < m_iMACAddrArraySize ; i++)
    {
        taf->setMACAddress(i,m_iParticipants[i]);
    }

    return taf;
    TMACPkt* mac = new TMACPkt(taf->getName(), taf->getKind());
    mac->setDestAddr(LAddress::L2BROADCAST);
    mac->setType(1);    //1 == TAF, 0 == Data
    ...
    if ((!useSCH) || (timeLeftInSlot() > sendingDuration)) {
        ...
        attachSignal((Mac80211Pkt*)mac, simTime()+RADIODELAY_11P, freq);
...
//          DBG_MAC << "Sending a Packet. Frequency " << freq << " Priority" << lastAC << std::endl;
        ...
        recordPacket(PassedMessage::OUTGOING, PassedMessage::LOWER_DATA, taf);

         //   sendDelayed(taf, RADIODELAY_11P, lowerLayerOut);
        char str[100] = "Send ";
        strcat(str,"TAF");
        findHost()->bubble("TAF");
        statsSentPackets++;
...
void TMAC::SendInCFP(cMsg* msg)
{
    channelBusySelf(true);
    cPacket* pktToSend = (cPacket*)msg->GetMessage();

    EV << "Message : " << msg->GetMessage()->getName() <<endl;
    //send the packet
    Mac80211Pkt* mac = new Mac80211Pkt(pktToSend->getName(), pktToSend->getKind());
    ...
    mac->setSrcAddr(myMacAddress);
    mac->encapsulate(pktToSend->dup());
    TAF*  taf =  dynamic_cast<TAF*>(mac->decapsulate());
...
void TMAC::OnTAF(TAF* taf)
{
//  EV <<   "SFD : "   <<  taf->getSuperframeDuration() << endl;
}
